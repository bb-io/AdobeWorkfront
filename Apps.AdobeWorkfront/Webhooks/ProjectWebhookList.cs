...
public class ProjectWebhookList(InvocationContext invocationContext) : BaseWebhookList(invocationContext)
{
    ...
    public Task<WebhookResponse<ProjectResponse>> OnProjectStatusChanged(WebhookRequest webhookRequest,
        [WebhookParameter] ProjectStatusOptionalRequest projectStatusOptionalRequest,
        [WebhookParameter] ProjectOptionalRequest projectOptionalRequest) => HandleWebhook<ProjectResponse>(webhookRequest, 
        payload => (projectStatusOptionalRequest.ProjectStatus == null ||
                    payload.NewState.Status.Equals(projectStatusOptionalRequest.ProjectStatus,
                        StringComparison.OrdinalIgnoreCase)) && 
                   (projectOptionalRequest.ProjectId == null || payload.NewState.ProjectId.Equals(projectOptionalRequest.ProjectId, StringComparison.OrdinalIgnoreCase)));

    [Webhook("On project changed", typeof(ProjectChangedHandler), Description = "Triggers when any property of a project changes")]
    public Task<WebhookResponse<ProjectResponse>> OnProjectChanged(WebhookRequest webhookRequest,
        [WebhookParameter] ProjectOptionalRequest projectOptionalRequest) => HandleWebhook<ProjectResponse>(webhookRequest, 
        payload => projectOptionalRequest.ProjectId == null || payload.NewState.ProjectId.Equals(projectOptionalRequest.ProjectId, StringComparison.OrdinalIgnoreCase));
